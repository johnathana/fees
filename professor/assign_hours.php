...
<head>
	<?php 
		...
		require_once($_SERVER['DOCUMENT_ROOT'].'/professor/form.libs.php');
	...
	<div class="content promos grid2col"> 
		...
		<div id="container">
			...
			<form id="myForm" name="myForm" action="assign_hours.php" method="POST">
				...
				<?php	while($row = mysql_fetch_assoc($result_set))
					{
						...
						echo "<tr>";
						echo "<td>$row[id]</td><td>$row[applied]</td><td>$row[student_name]</td><td>$row[student_email]</td><td>$workoffer_row[professor_name]</td><td>$workoffer_row[title]</td><td>$workoffer_row[hours]</td><td>$workoffer_row[start_date]</td><td>$workoffer_row[end_date]</td><td>$row[hours_accepted]</td>";
						echo "</tr>";
					}
				...
				<br />
				<p>
					<input class="button" type="submit" name="submit_btn" value="Καταχώρηση ωρών"  />
